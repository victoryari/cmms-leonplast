...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center space-x-3">
        <a href="{{ route('activos.index') }}" class="p-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white transition">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
        </a>
        <div>
            <h2 class="text-2xl font-extrabold text-white">Alta de Nuevo Activo Industrial</h2>
            <p class="text-xs text-slate-400">Ingresa las especificaciones del equipo para incorporarlo al inventario y autogenerar su código QR</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('activos.store') }}" class="space-y-6">
            ...
            <div>
                <h3 class="text-xs font-bold text-blue-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <span>1. Información Principal del Equipo</span>
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        <label for="nombre" class="block text-xs font-semibold text-slate-300 mb-1">Nombre Completo del Activo *</label>
                        <input type="text" id="nombre" name="nombre" value="{{ old('nombre') }}" required placeholder="Ej: Inyectora de Plástico Engel 350T"
                    ...
                    <div>
                        ...
                        <select id="categoria" name="categoria" required 
                            ...
                            <option value="{{ $cat->nombre }}" {{ old('categoria') == $cat->nombre ? 'selected' : '' }}>{{ $cat->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="estado_operativo" name="estado_operativo" required 
                                class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                            <option value="Operativo" {{ old('estado_operativo') == 'Operativo' ? 'selected' : '' }}>Operativo</option>
                            <option value="Mantenimiento" {{ old('estado_operativo') == 'Mantenimiento' ? 'selected' : '' }}>En Mantenimiento</option>
                            <option value="Reparacion" {{ old('estado_operativo') == 'Reparacion' ? 'selected' : '' }}>En Reparación</option>
                            <option value="Fuera_de_servicio" {{ old('estado_operativo') == 'Fuera_de_servicio' ? 'selected' : '' }}>Fuera de Servicio</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-800">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="ubicacion" name="ubicacion" value="{{ old('ubicacion') }}" placeholder="Ej: Nave A - Inyección"
                               class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500">
                    </div>

                    <div>
                        <label for="area" class="block text-xs font-semibold text-slate-300 mb-1">Área Operativa</label>
                        <input type="text" id="area" name="area" value="{{ old('area') }}" placeholder="Ej: Producción Inyección"
                               class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500">
                    </div>
            ...
            <!-- Section 3: Description & Acquisition -->
            <div class="pt-4 border-t border-slate-800">
                <h3 class="text-xs font-bold text-emerald-400 uppercase tracking-wider mb-4">3. Descripción y Adquisición</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        <label for="fecha_adquisicion" class="block text-xs font-semibold text-slate-300 mb-1">Fecha Adquisición</label>
                    ...
                    <div>
                        ...
                        <input type="number" id="vida_util_estimada" name="vida_util_estimada" value="{{ old('vida_util_estimada', 15) }}"
                               class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500">
                    </div>
                </div>

                <div>
                    <label for="descripcion" class="block text-xs font-semibold text-slate-300 mb-1">Descripción Técnica Detallada</label>
                    <textarea id="descripcion" name="descripcion" rows="3" placeholder="Describe las funciones principales y características técnicas..."
            ...
            <div class="pt-4 border-t border-slate-800 flex items-center justify-end space-x-3">
                <a href="{{ route('activos.index') }}" class="px-5 py-2.5 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 text-xs font-semibold transition">
                    Cancelar
                </a>
