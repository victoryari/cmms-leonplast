...
@section('title', 'Catálogo de Activos Digitales')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="{{ route('activos.create', ['tipo_clasificacion' => 'Digital']) }}" 
           class="inline-flex items-center justify-center space-x-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-extrabold text-xs shadow-lg shadow-blue-600/30 transition transform active:scale-95">
            ...
            <span>+ Registrar Activo Digital</span>
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800">
            <p class="text-[11px] font-semibold text-slate-400 uppercase">Total Activos Digitales</p>
            <p class="text-2xl font-extrabold text-white mt-1">{{ $metrics['total'] }}</p>
        </div>
        <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800">
            <p class="text-[11px] font-semibold text-emerald-400 uppercase">Licencias / Firmware Activo</p>
            <p class="text-2xl font-extrabold text-emerald-400 mt-1">{{ $metrics['operativos'] }}</p>
        </div>
        <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800">
            <p class="text-[11px] font-semibold text-amber-400 uppercase">En Revisión / Renovación</p>
            <p class="text-2xl font-extrabold text-amber-400 mt-1">{{ $metrics['revision'] }}</p>
        </div>
    </div>
    ...
    <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center space-x-2 text-xs text-slate-400 font-medium">
            <span>Repositorio de licencias de software industrial, recetas PLC, firmwares y esquemas CAD/CAM</span>
        </div>

        <form method="GET" action="{{ route('activos.digitales') }}" class="flex items-center space-x-2 w-full sm:w-auto">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar por código, nombre o versión..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-slate-300 border-collapse">
                <thead class="bg-slate-950/90 text-slate-400 font-semibold border-b border-slate-800 text-[11px]">
                    <tr>
                        <th class="py-3 px-4">Código / ID Digital</th>
                        <th class="py-3 px-4">Nombre del Activo Digital</th>
                        <th class="py-3 px-4">Categoría / Tipo</th>
                        <th class="py-3 px-4">Versión / Firmware</th>
                        <th class="py-3 px-4">Servidor / Maquinaria</th>
                ...
                <tbody class="divide-y divide-slate-800/60 font-medium">
                    ...
                    <tr class="hover:bg-slate-800/40 transition group">
                        ...
                        <td class="py-3 px-4">
                            ...
                            <span class="block text-[10px] text-slate-500">{{ $a->marca ?? 'León Plast Digital' }}</span>
                        </td>
                        <td class="py-3 px-4 text-slate-300">
                            {{ $a->categoria }}
                        </td>
                        <td class="py-3 px-4 font-mono text-[11px] text-amber-400">
                            {{ $a->modelo ?? 'v1.0' }}
                        </td>
                        <td class="py-3 px-4 text-slate-400">
                            {{ $a->ubicacion ?? 'Servidor Central' }}
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="py-8 text-center text-slate-500 text-xs">
                            No se encontraron activos digitales registrados.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
